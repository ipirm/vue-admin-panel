<template>
  <div class="card-page">
    ...
    <alert-info 
      text="Manage operators by controlling who has access to what products and tabs within the Booking Tours Management Center."
      button="Add Operator"
      @buttonClick="$router.push('/operators/new')" />
    ...
    <entity-table @delete-user="deleteOperatorEmit" @click-arrow="onSort" />

    <div class="pagination-cont">
      <pagination-table :meta="operatorsMeta" @paginate="paginate" />
    </div>
      
    <base-modal :active="activeModal" @close="activeModal = false">
      <div class="confirmation">Confirmation</div>
      <div class="question">Are you sure to delete a user?</div>
...
import EntityTable from '@/components/UILibrary/Tables/EntityTableOperators'
...
export default {
  ...
  computed: {
    ...mapState('ui', ['loading']),
    ...mapState('operators', ['operatorsMeta'])
  },
  methods: {
    ...mapActions('operators', ['getFilterOperators', 'deleteOperator']),

    deleteOperatorEmit(item) {
    ...
    modalClickDelete() {
      this.deleteOperator(this.deleteId)
    ...
    get() {
      this.getFilterOperators(this.queryParams)
    }
  }
}
</script>
