        ...
        <div class="table">
            ...
            <div class="table-items">
                <transition-group name="fade" tag="div">
                    <div v-for="(item, key) in filterTypes" :key="key + 0" class="table-item">
                        ...
                        <div class="field actions">
                            <router-link :to="'product-types/' + item.id">
                                <div class="action">
                                    <svg-icon class="icon-action" name="act_edit" />
                                </div>
                            </router-link>

                            <router-link :to="'product-types-categories/' + item.id">
                                <div class="action">
                                    <svg-icon class="icon-action" name="plus" />
                                </div>
                            </router-link>

                            <div class="action" @click="$emit('delete-type', item)">
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                tableHead: [
                    ...
                    { class: 'type', name: 'Type', active: false },
        ...
        computed: {
            ...mapState('types', ['filterTypes'])
        },

        watch: {
            checkAll() {
                this.checkedItemIds = this.checkAll ? this.filterTypes.map(item => item.id) : []
            }
        },
