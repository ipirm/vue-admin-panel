        ...
        <div class="table">
            ...
            <div class="table-items">
                <transition-group name="fade" tag="div">
                    <div v-for="(item, key) in filterMenus" :key="key + 0" class="table-item">
                        ...
                        <div class="field location"> {{ item.location }} </div>
                        <div class="field actions">
                            <router-link :to="'menus/' + item.id">
                                <div class="action">
                                    <svg-icon class="icon-action" name="act_edit" />
                                </div>
                            </router-link>

                            <div class="action" @click="$emit('delete-menu', item)">
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                tableHead: [
                    ...
                    { class: 'location', name: 'Location', active: false },
                    { class: 'title', name: 'Title', active: false },
        ...
        computed: {
            ...mapState('menus', ['filterMenus'])
        },

        watch: {
            checkAll() {
                this.checkedItemIds = this.checkAll ? this.filterMenus.map(item => item.id) : []
            }
        },
