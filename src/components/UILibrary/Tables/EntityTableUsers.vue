    ...
    <div class="table">
      ...
      <div class="table-items">
        <transition-group name="fade" tag="div">
          <div v-for="(item, key) in filterUsers" :key="key + 0" class="table-item">
            ...
            <div class="field role"> {{ item.role }} </div>
            <div class="field email"> {{ item.email }} </div>
            <div class="field full_name"> {{ item.profile && item.profile.full_name && '' }} </div>
...
export default {
  data() {
    return {
      ...
      tableHead: [
        ...
        { class: 'role', name: 'Role', active: false },
        { class: 'email', name: 'Email', active: false },
        { class: 'full_name', name: 'Full Name', active: false },
  ...
  computed: {
    ...mapState('users', ['filterUsers'])
  },

  watch: {
    checkAll() {
      this.checkedItemIds = this.checkAll ? this.filterUsers.map(item => item.id) : []
    }
  },
