        ...
        <div class="table">
            ...
            <div class="table-items">
                <transition-group name="fade" tag="div">
                    <div v-for="(item, key) in filterSettings" :key="key + 0" class="table-item">
                        ...
                        <div class="field type"> {{ item.typeName }} </div>
                        <div class="field name"> {{ item.name }} </div>
                        <div class="field description"> {{ item.description }} </div>
                        <div class="field actions">
                            <router-link :to="'product-setting-types/' + item.id">
                                <div class="action">
                                    <svg-icon class="icon-action" name="act_edit" />
                                </div>
                            </router-link>

                            <router-link :to="'product-setting-types-categories/' + item.id">
                                <div class="action">
                                    <svg-icon class="icon-action" name="plus" />
                                </div>
                            </router-link>

                            <div class="action" @click="$emit('delete-setting', item)">
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                tableHead: [
                    ...
                    { class: 'type', name: 'Type Name', active: false },
                    { class: 'name', name: 'Name', active: false },
                    { class: 'description', name: 'Description', active: false },
        ...
        computed: {
            ...mapState('settings', ['filterSettings'])
        },

        watch: {
            checkAll() {
                this.checkedItemIds = this.checkAll ? this.filterSettings.map(item => item.id) : []
            }
        },
