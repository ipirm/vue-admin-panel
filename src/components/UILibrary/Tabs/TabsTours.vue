    ...
    <ul class="nav">
      <li class="nav-item" v-for="item in navItems" v-bind:key="item.title">
       ...
       <div v-for="subNav in item.subcategories" v-bind:key="subNav.stepNumber" class="navbar-dropdown">
         <a :class="{'nav-item-active': subNav.isActive,'nav-item-completed': subNav.isCompleted, 'nav-item-not-required': subNav.notRequired}" class="dropdown-item" @click.stop="switchStep(subNav.stepNumber)">
           <svg-icon v-if="(subNav.isCompleted || subNav.notRequired) && !subNav.isActive" name="completed"></svg-icon>
...
//                }
        ...
        beforeMount(){
            let compContext = this;
            Object.keys(this.tour.progress.steps).forEach(function(key) {
                if(key == 'INPUT_LANGUAGE_AND_TITLE'){
                    if(compContext.tour.progress.steps[key] == 1){
                        compContext.navItems[0].subcategories[0].isCompleted = true;
                    }else{
                        compContext.navItems[0].subcategories[0].isCompleted = false;
                    }
